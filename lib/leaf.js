...
var Leaf = createReactClass({
  ...
  render: function () {
    ...
    var onLabelMouseEnter = this._onMouseEnter.bind(this, d);
    var onLabelMouseLeave = this._onMouseLeave.bind(this, d);

    let labelClassName = 'json-inspector__line';
    const bold = this.props.isBold(this.keypath(), p.data);
    if (bold) {
      labelClassName += ' json-inspector__line--bold';
    }
    const highlight = this.props.isHighlight(this.keypath(), p.data);
    if (highlight) {
      labelClassName += ' json-inspector__line--highlight';
    }
    ...
    return h(
      ...
      h(
        ...
        h(
          'span',
          { className: 'json-inspector__key', onMouseEnter: onLabelMouseEnter, onMouseLeave: onLabelMouseLeave },
          this.format(d.key),
          ':',
          this.renderInteractiveLabel(d.key, true)
        ),
        this.renderTitle({ onMouseEnter: onLabelMouseEnter, onMouseLeave: onLabelMouseLeave }),
        this.renderShowOriginalButton()
      ),
      this.renderChildren()
    );
  },
  renderTitle: function ({ onMouseEnter, onMouseLeave }) {
    var data = this.data();
    var t = type(data);
    ...
    switch (t) {
      ...
      default:
        return h(
          'span',
          { className: 'json-inspector__value json-inspector__value_' + t.toLowerCase(), onMouseEnter, onMouseLeave },
  ...
  _onMouseEnter: function (data, e) {
    if (this.props.onMouseEnter) {
      this.props.onMouseEnter(data);
    }
    e.stopPropagation();
  },
  _onMouseLeave: function (data, e) {
    if (this.props.onMouseLeave) {
      this.props.onMouseLeave(data);
    }
    e.stopPropagation();
  },
